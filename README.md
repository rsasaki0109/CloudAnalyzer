...
| `ca slam-run` | "Drive a LiDAR SLAM on raw scans and (optionally) score the result." | Directory of `.bin`/`.pcd`/`.ply` sweeps (or a frames-list `.txt`) + optional reference map / trajectory | KISS-ICP odometry runner (default) plus an experimental `--driver kiss-slam` for KISS-ICP + pose-graph + loop-closures; `--evaluate` pipes the result into `ca run-evaluate`; install with `pip install 'cloudanalyzer[slam]'` |
